...
class Environment:
    def __init__(self, str_maze: str, brickwalls: bool = False):
    ...
    def is_wall(self, state: tuple):
        return state in self.__states and self.__states[state] == MAZE_WALL
    ...
    def do(self, state: tuple, action: str):
        ...
        if self.is_forbidden_state(new_state):
            ...
            if self.is_wall(new_state) and self.__brickwalls:
                state = new_state
        else:
            state = new_state
